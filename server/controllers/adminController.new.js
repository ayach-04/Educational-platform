...
const getDashboardStats = async (req, res) => {
  try {
    ...
    // Count total chapters across all modules
    const modulesWithChapters = await Module.find({}, 'chapters');
    const totalChapters = modulesWithChapters.reduce((total, module) => {
...
const deleteUser = async (req, res) => {
  try {
    ...
    // Delete modules created by this user (if any)
    try {
      const modules = await Module.find({ createdBy: user._id });

      // Delete files from local storage for each module
      for (const module of modules) {
            ...
            if (chapter.files && chapter.files.length > 0) {
              for (const file of chapter.files) {
                try {
                  ...
                  console.error(`Error deleting file from local storage: ${file.path}`, fileError);
        ...
        if (module.syllabus && module.syllabus.files && module.syllabus.files.length > 0) {
          for (const file of module.syllabus.files) {
            try {
              const filePath = path.join(__dirname, '..', file.path);
              if (fs.existsSync(filePath)) {
                fs.unlinkSync(filePath);
              ...
              console.error(`Error deleting syllabus file: ${file.path}`, fileError);
            }
          }
        }
            ...
            if (reference.files && reference.files.length > 0) {
              for (const file of reference.files) {
                try {
                  ...
                  console.error(`Error deleting reference file: ${file.path}`, fileError);
                }
              }
            }
          }
      ...
      // Delete the modules
      await Module.deleteMany({ createdBy: user._id });
    } catch (moduleError) {
      console.error('Error handling modules during user deletion:', moduleError);
      // Continue with user deletion even if module handling fails
    }
...
const deleteModule = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Error handling files during module deletion:', fileError);
      // Continue with module deletion even if file handling fails
    }
